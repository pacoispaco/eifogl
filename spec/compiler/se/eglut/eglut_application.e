...
feature {NONE} -- Initialization
	...
	argument_array: ARRAY [STRING] is
		do
			!! Result.from_collection (command_arguments)
		end
...
--			"glutGetModifiers ()"
	...
	eglut_init (argc: INTEGER; argv: ARRAY [STRING]) is
			...
			-- the GLUT library, and thus may modify both `argc'
		...
		do
			eglut_init_i := argc
		        ceif_adopt (argv)
			glut_init ($eglut_init_i, argv.to_external)
			ceif_wean (argv)
		end
	
	new_eglut_init (argc: INTEGER; argv: ARRAY [STRING]) is
			-- Initialize the GLUT library. Note that GLUT library
			-- will extract any command line options intended for
		...
		local
			cs: CWRAP_STRINGS
		do
			!! cs.make_from_array (argv)
--		        cs.dump
			glut_init (cs.count_pointer, cs.strings_pointer)
--			cs.dump
		end
...
feature {NONE} -- Implementation (EGLUT C interface)	
	...
	frozen eglut_set_application_object (p: like Current) is
			-- Set the application object in the EGLUT C interface
		external
